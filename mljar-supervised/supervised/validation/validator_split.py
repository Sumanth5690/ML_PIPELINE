...
class SplitValidator(BaseValidator):
    ...
    def get_split(self, k=0, repeat=0):
        ...
        # Check if validation set has only one class (for classification tasks)
        if stratify is not None and len(np.unique(y_validation)) == 1:
            log.warning(f"Validation set contains only one class. Adjusting train_ratio from {self.train_ratio} to increase validation set size.")
            
            # Try with a larger validation set (smaller train_ratio)
            adjusted_train_ratio = max(0.5, self.train_ratio - 0.2)
            
            output_data = train_test_split(
                *input_data,
                train_size=adjusted_train_ratio,
                test_size=1.0 - adjusted_train_ratio,
                shuffle=self.shuffle,
                stratify=stratify,
                random_state=self.random_seed + repeat,
            )
            ...
            # If still only one class, disable stratification and try again
            if len(np.unique(y_validation)) == 1:
                log.warning("Still single class after adjusting ratio. Disabling stratification for this split.")
                
                input_data_no_stratify = [X, y]
                if sample_weight is not None:
                    input_data_no_stratify += [sample_weight]
                if sensitive_features is not None:
                    input_data_no_stratify += [sensitive_features]
                
                output_data = train_test_split(
                    *input_data_no_stratify,
                    train_size=adjusted_train_ratio,
                    test_size=1.0 - adjusted_train_ratio,
                    shuffle=self.shuffle,
