...
def logloss(y_true, y_predicted, sample_weight=None):
    ...
    try:
        ...
        if "y_true contains only one label" in str(e):
            # Handle case where validation set contains only one class
            # Return a high loss value to indicate poor performance
            log.warning(f"Validation set contains only one class: {np.unique(y_true)}. Returning high loss value.")
            return 10.0  # High loss value to indicate this is not a good split
        else:
            raise e
